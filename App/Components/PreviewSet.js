...
export class PreviewSession extends React.Component {
  ...
  rebuildSession = () => {
    ...
    this.setState({
      session: session,
      success: session.Exercices[this.state.exerciceKey].success,
    });
  ...
  getColor = boolValue => {
    if (boolValue) return StyleElements.background.backgroundColor;
    return 'grey';
  };
    ...
    return (
      <View style={styles.button}>
        ...
        <View style={{flex: 1, flexDirection: 'row'}}>
          <TouchableOpacity
            style={{height: 100, width: 100, marginTop: 5, marginLeft: 5}}
            onPress={() => navigateToScreen(parent, 'ExoNote', null)}>
            <Image
          ...
          <View style={{flex: 1, flexDirection: 'column'}}>
            <Text style={{marginTop: 5, marginLeft: 100}}>
              {this.props.sets} {' x '} {this.props.nbrRep}{' '}
            </Text>
            <Text style={{marginTop: 5, marginLeft: 115}}>
              {' '}
              {this.props.weith} {' Kg'}{' '}
            </Text>
          </View>
        ...
        <View style={{flex: 1}}>
          <ScrollView
            horizontal={true}
            style={{marginTop: 25, marginLeft: 110, flexDirection: 'row'}}>
            {this.state.session.Exercices[this.state.exerciceKey].success.map(
              success => (
