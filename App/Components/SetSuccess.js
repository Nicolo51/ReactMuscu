...
export class SetSuccess extends React.Component{
   ...
   renderNumber = (value) => {
       ...
       if (value < 100){
        ...
        return(<Image style={{ height: 20, width: 20, marginTop: -20 }} source={Images.getImage(false)}/>)
       }
   }
   renderFailOrSuccess = (value) => {
       if(value == parseInt(value, 10)){
           return (<View style={{height: 20, width:20, marginLeft: 10 }}>
               <Image style={{ height: 20, width: 20}} source={Images.getImage(null)}/>
               {this.renderNumber(value)}
           </View>)

       }
       else {
           return (<Image style={{ height: 20, width: 20, marginLeft: 10 }} source={Images.getImage(value)}/>)
       }
