...
export class SetsList extends React.Component {
  ...
  static navigationOptions = {
    ...
    header: props => (
      <Header
        ...
        onButtonPress={() => self.onAddSessionPress()}
        tabName={'Set List Screen'}
  ...
  addSession = (name, muscle, numberOfRep, timer, image) => {
    ...
    session.Exercices.push({
      key: this.state.session.Exercices.length,
      name: name,
      muscle: muscle,
      nbrRep: numberOfRep,
      restTime: timer,
  ...
  onAddSessionPress = () => {
    navigateToScreen(this, 'CreateSet', {
      onGoBack: (name, muscle, numberOfRep, timer, image) =>
        this.addSession(name, muscle, numberOfRep, timer, image),
    });
  };
  ...
  render() {
    return (
      <View style={{flex: 1}}>
        <DialogInput
          isDialogVisible={this.state.isFailVisible}
          title={'Raté pour cette fois :('}
          message={'Combien de rep avez vous réussi a faire ?'}
          submitInput={inputText => {
            this.setDone(inputText, this.state.selectedSet);
          }}
          closeDialog={() => {
            this.setState({isFailVisible: false});
          }}
        ...
        <View style={{flexDirection: 'row', height: 75}}>
          ...
          <TouchableOpacity
            ...
            onPress={() => this.setDone(true, this.state.selectedSet)}>
            <Image
              style={{height: 50, width: 50}}
              source={Images.getImage('check_ico')}
            />
          </TouchableOpacity>
          <TouchableOpacity
            style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}
            onPress={() => this.setDone(false, this.state.selectedSet)}>
            <Image
              style={{height: 50, width: 50}}
              source={Images.getImage('uncheck_ico')}
            />
          </TouchableOpacity>
        </View>
      </View>
